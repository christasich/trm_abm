...
class auction(object):
    # initialize the auction object using self and hh_dict, a dictionary of all the households
    def __init__(self, hh_dict):
        self.households = hh_dict
        self.transactions = []
        self.ballots = None
    ...
    def utility(self, index):
        # create a dictionary of house hold ids and locations
        u = dict( [ (hh.id, hh.eu_df.eu.loc[index]) for hh in self.households.values() ] )
        # create a dictionary of transactions if they correspond to the index given
        trans = [t for t in self.transactions if t.year == index]
        # for each trasaction in transactions, subtract the price from price corresponding to the buyer's id and add it to the price correspoinding to the seller's id
        for t in trans:
        ...
        return(u) # return the updated u dictionatry
    ...
    def vote(self, force = False):
        if self.ballots is None:
            return None
        majority = 0.5 * self.ballots.shape[0] # majority is 50% of the number of rows in the ballots array
        votes = pd.DataFrame(self.ballots)[0].value_counts() # creating a dataframe containing the counts of different values
        print(votes) # print the dataframe votes
        # if the highest number of votes is greater than the majority, return the value associated with it
        if votes.iloc[0] > majority:
            return votes.index[0]
        # if force is true, return the plurality
        elif force:
            return votes.index[0]
    ...
    def initialize_votes(self):
        # create a NumPy array out of the households dictionary ignoring the index column after all the hosueholds vote
        self.ballots = np.array( [ hh.vote() for hh in self.households.values() ] )
        self.transactions = [] # creating an empty list of transactions

    def auction(self, max_rounds = 1000):
        global tt, bbids # variables for debugging
        self.initialize_votes() #
        for round in range(max_rounds):
            ...
            if winner is not None:
                return (winner, self.utility(winner))
            # set variable target equal to the majority or the plurality
            target = self.vote(force = True)
            # print "Target = ", target
            # create a list called buyers containing all the buyers
            buyers  = [trans.buyer for trans in self.transactions]
            # create a list called sellers contianing all the sellers
            sellers = [trans.seller for trans in self.transactions]
            # create a list called buyer_ids containing all the buyers' ids
            buyer_ids = [ b.id for b in buyers ]
            # create a list called sellers_ids containing all the sellers' ids
            seller_ids = [ s.id for s in sellers ]
            # create a list called neutral_ids making a list out of the keys in the dictionary of households, removing all the ids of the buyers and sellers, and remoiving duplicates
            neutral_ids = list(set(self.households.keys()) - set(buyer_ids) - set(seller_ids))
            # create a list called neutral containing all in information about the households in the list neutral_ids
            neutral = [ self.households[hh_id] for hh_id in neutral_ids ]
            # for all neutral households, construct bids using the number of years most farmers want to keep the breach open
            for hh in neutral:
                hh.construct_bids(target)
            # for all households in buyers, construct bids using the purchases list and the number of years most farmers want to keep the breach open
            for hh in buyers:
                # set purchases equal to a list of transactions in the self.transactions list if the buyer's id is equal to the household's id
                purchases = [ t for t in self.transactions if t.buyer_id() == hh.id ]
                hh.construct_bids(target, purchases)
            # create a concatinated array of bids for both neutral and buyers
            bids = pd.concat([hh.bids for hh in neutral + buyers])
            # create an array bbids that is a copy of the array bids
            bbids = bids.copy()
            # go through a bidding rounds using the array of bids from neutral and buyers and set transactions equal to the list of those bids
    ...
    def bidding_round(self,bids):
        ...
        # set transactions equal to an empty list
        transactions = []
        # set wta (willing to accept) equal to the logical_not (the true/false opposite) of 
        wta = bids[np.logical_not(bids.is_offer.values)][['id', 'year','amount']]
        # set wtp (willing to pay)
...
class household(object):
    ...
    def construct_bids(self, target, purchases = []):
        bid_scale = 1.1     # 1.1 = 10% variation
        # if expeced utility dataframe is None, produce an error
        assert(self.eu_df is not None)
        # set favorite to the one with the highest expected utility
        favorite = self.eu_df.iloc[0]
        # set others to a list containing all the other expected utilities 
        others = self.eu_df.iloc[1:]
        # print "Target = ", target
        # if the number of years the favorite wants to keep the breach open is equal to the target, set the target equal to the number of years that the farmer with the second highest utility wants to keep the breach open
        if target == favorite.year:
            target = self.eu_df.index[1]
        # set target_eu to the row of the eu_df that corresponds to target
...
class polder(object):
    ...
    def set_elevation_cube(self, elevation_cube, plots, n_households = None):
        if n_households is None:
            n_households = len(self.households)
        self.elevation_cube = elevation_cube.copy()
        self.elevation = self.elevation_cube[0]
        self.owners = np.zeros_like(self.elevation, dtype = np.integer)
        self.plots = plots
        self.initialize_hh_from_plots(n_households)
        self.current_period = self.elevation_cube.shape[0]
    ...
    def initialize_hh_from_plots(self, n_households):
        assert self.owners.max() < n_households
